...
def recommend_places(place_types=None, borough=None, facets=None):
    """
    ...
    if place_types:
        # Create a regex pattern to match any of the selected types
        place_types_pattern = '|'.join(map(re.escape, place_types))
        recommendations = recommendations[recommendations["type"].str.contains(place_types_pattern, case=False, na=False, regex=True)]
...
def recommend():
    ...
    if request.method == "POST":
        place_types = request.form.getlist('type')  # Get all selected types
        borough = request.form.get('borough')
        selected_facets = request.form.getlist('facets')

        exact_matches, partial_matches = recommend_places(place_types, borough, selected_facets)
...
def autocomplete():
    try:
        ...
        # Normalize the place names in the DataFrame
        df['normalized_name'] = df['name'].str.strip().str.lower()

        # Filter place names based on the query
        matches = df[df['normalized_name'].str.contains(query, na=False)]
